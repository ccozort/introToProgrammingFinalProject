...
# COZORT: don't need this because you're using it in the for loop
# debris = Debris()

# COZORT: don't use colorbyte function because it's not really needed.  Used WHITE for now, as the color just needs an RGB value, like (255,255,255)
for i in range(100):
    m = Debris(randint(0, WIDTH), randint(0, HEIGHT), 25, 25, (WHITE))
    all_sprites.add(m)
    # COZORT: Need to add debris as a sprite group so pygame can handle it as a group of data
    debris.add(m)
    print(m)
